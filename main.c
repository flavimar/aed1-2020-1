...
int gerarAlimento(SDL_Renderer *renderer,Objeto *alimento){
    ...
    int x = (((rand() % (larguratela / tamCobra))) * tamCobra) - tamCobra;
    int y = (((rand() % (alturatela / tamCobra)))* tamCobra) - tamCobra;
    printf("%d %d\n",x,y);
   ...
   int colisaoAlimento(Objeto *alimento,Fila *cobra){
    ...
    if(cobra->inicio->reg.rect.x == alimento->reg.rect.x){
       ...
       if(cobra->inicio->reg.rect.y == alimento->reg.rect.y){
    	...
    	aumentaCobra(cobra);
    	printf("contador: %d -- Ultimo: %d\n",contTamCobra(cobra),cobra->fim->dir);
    	inicializaAlimento(alimento);
        return 1;
       }
    }
